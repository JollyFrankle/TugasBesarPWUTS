...
<div class="body d-flex justify-content-between align-items-center">
    ...
    <a href="./CekPeminjaman.php" class="btn btn-success">Kembali</a>
</div>
    ...
    <table class="table table-hover rounded rounded-1 overflow-hidden">
        ...
        <tbody class="table-group-divider">
            ...
            if (mysqli_num_rows($query) == 0) {
                echo '<tr> <td colspan="7"> Tidak ada peminjam aktif </td> </tr>';
            } else {
                $no = 1;
                while ($data = mysqli_fetch_assoc($query)) { ?>
                    <tr class="<?php echo (date("Y-m-d") > $data['tanggal_kembali']) ? 'table-danger' : '';?>">
                        <th scope="row"><?php echo $no; ?></th>
                        <td><?php echo $data['nama']; ?></td>
                        <td class="text-center"><?php echo date('j F Y', strtotime($data['tanggal_pinjam'])); ?></td>
                        <td class="text-center">
                            <?php echo date('j F Y', strtotime($data['tanggal_kembali'])); ?>
                        <?php if(date("Y-m-d") > $data['tanggal_kembali']) { ?>
                            <div class="badge text-bg-danger">Overdue!</div>
                        <?php } ?>
                        </td>
                    </tr>
            <?php $no++;
